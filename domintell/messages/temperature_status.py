...
class GenericTemperaturetatusMessage(domintell.Message):
    ...
    def populate(self, serialNumber, dataType, dataString):
        """
        Populate temperature values from T/U messages.
        Ignore PBL button (B) and output (O) messages.
        """
        ...
        # Only PBL temperature messages contain T or U
        if dataType not in ("T", "U"):
            return  # ignore non-temperature PBL messages

        # Expect exactly 4 values: current, set_point, mode, range
        if len(data) != 4:
            return  # ignore malformed temperature messages
...
class PBLTemperaturetatusMessage(GenericTemperaturetatusMessage):
    def __init__(self, address=None):
        GenericTemperaturetatusMessage.__init__(self)
        self.moduleType = PBL_COMMAND_CODE
    ...
    def populate(self, serialNumber, dataType, dataString):
        ...
        # Ignore button press and output messages
        if dataType in ("B", "O"):
            return

        # Only parse temperature packets (T or U)
        super().populate(serialNumber, dataType, dataString)
...
domintell.register_command(PBL_COMMAND_CODE, PBLTemperaturetatusMessage)
