...
class DomintellParser(object):
    ...
    def feed(self, data):
        """
        Parse Domintell UDP packets. Some modules (like PBL) send multiple
        messages inside a single UDP datagram, separated by CRLF.
        """

        # Decode using correct encoding
        decoded = data.decode('iso8859_13').lstrip('\r\n ')

        # Ignore empty or too short packets
        if len(decoded) < 10:
            return

        # If multiple Domintell messages exist in one UDP packet, split them
        lines = decoded.split("\r\n")
        for line in lines:
            line = line.strip()
            if not line:
            ...
            # Each line is an individual Domintell command
    ...
    def parse(self, data):
        ...
        if self._mode == APP_INFO_MODE:
            ...
            if module_type in domintell.CommandRegistry:
                ...
                if module_type == "PBL" and "\n" in data_string:
                    lines = data_string.splitlines()
                    first = lines[0].strip()
                    data_parts = first.split()
                else:
                    data_parts = data_string.split()

                clean_data = " ".join(data_parts)
                message.populate(serial_number, data_type, clean_data)
